...
from Alpha101.utils import *
class FormulaicAlphas(object):
    def __init__(self, df, AlphaModel_name):
        self.AlphaModel_name = 'self.alpha' + AlphaModel_name

        self.df_data = df.copy()
        self.open = self.df_data['Open']
        self.high = self.df_data['High']
        self.low = self.df_data['Low']
        self.close = self.df_data['Close']
        self.volume = self.df_data['Volume'] * 100
        self.returns = (np.log(self.df_data['Open'] / self.df_data['Open'].shift()))
        self.returns[0] = self.returns[1]
        # self.vwap = (df_data['S_DQ_AMOUNT']*1000)/(df_data['S_DQ_VOLUME']*100+1)
        self.df_data['weights'] = self.alpha003()
